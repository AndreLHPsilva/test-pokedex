  ...
  static validate(data: any): IData {
    const getPokemonSchema = z.object({
      limit: z.string({
        ...
        invalid_type_error: "Parâmetro Limit deve ser do tipo string ou number",
      })
      .refine(value => {
        const parsedValue = parseInt(value, 10);
        return !isNaN(parsedValue);
      }, { message: "Parâmetro Limit deve ser um número válido" }),
      offset: z.string({
        required_error: "Parâmetro offset é obrigatório.",
        invalid_type_error: "Parâmetro offset deve ser do tipo string ou number",
      })
      .refine(value => {
        const parsedValue = parseInt(value, 10);
        return !isNaN(parsedValue);
